...
import com.risenb.witness.ui.tasklist.TaskListSchedule;
...
public class MyReceiver extends BroadcastReceiver {
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
            ...
            if (!TextUtils.isEmpty(message) && message.length() > 2) {
                switch (message.substring(0, 2)) {
                    case "驳回":
                        i = new Intent(context, TaskListSchedule.class);
                        break;
                    case "推送":
                        i = new Intent(context, TaskListSchedule.class);
                        break;
                    default:
                        i = new Intent(context, VipMessageUI.class);
                        break;
                }
                i.putExtras(bundle);
    ...
    private void processCustomMessage(Context context, Bundle bundle) {
        if (HomeUI.isForeground) {
            String message = bundle.getString(JPushInterface.EXTRA_MESSAGE);
            String extras = bundle.getString(JPushInterface.EXTRA_EXTRA);
            Intent msgIntent = new Intent(HomeUI.MESSAGE_RECEIVED_ACTION);
            msgIntent.putExtra(HomeUI.KEY_MESSAGE, message);
            if (!JPushUtil.isEmpty(extras)) {
                try {
                    JSONObject extraJson = new JSONObject(extras);
                    if (extraJson.length() > 0) {
                        msgIntent.putExtra(HomeUI.KEY_EXTRAS, extras);
                    }
                } catch (JSONException e) {
