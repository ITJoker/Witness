...
//    }
    ...
    public static Bitmap stringToBitmap(String text, int size, int color, Typeface font, int bgColor) {
        ...
        float space = MyApplication.getInstance().getResources().getDimensionPixelSize(R.dimen.dm002);  //背景padding

        int textHeight = (int) getTextHeight(p);    //字的高度
        int pixLen = (int) (getFontHeight(p) + 2 * space);//正方形边长

        int offsetY = MyApplication.getInstance().getResources().getDimensionPixelSize(R.dimen.dm002);    //y方向上的偏移  手动调整
    ...
    public static float getLineSpaceHeight() {
        return MyApplication.getInstance().getResources().getDimensionPixelSize(R.dimen.dm004);
    }
    ...
    public static float getCenterY(Paint paint) {
        int offset = MyApplication.getInstance().getResources().getDimensionPixelSize(R.dimen.dm002);
