...
public class WillExpireFragment extends BaseLazyFragment {
    ...
    public static WillExpireFragment newInstance(String mark) {
        WillExpireFragment lf = new WillExpireFragment();
            ...
            public void onSuccess(int statusCode, BaseBeans<List<TaskListBean.TaskList>> response) {
                ...
                if (null != data && data.size() > 0) {
                    if (pagecount == 1) {
                        ...
                        mWeAdapter = new RecentlyDistanceAdapter(mActivity, willExpireSearchAddressList, layoutmark);
                        mWeAdapter.setOnItemClickListener(new OnRecyclerViewItemClickListener() {
                            @Override
