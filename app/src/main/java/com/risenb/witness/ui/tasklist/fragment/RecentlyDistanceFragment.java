...
public class RecentlyDistanceFragment extends BaseLazyFragment {
    ...
    public static RecentlyDistanceFragment newInstance(String mark) {
        RecentlyDistanceFragment lf = new RecentlyDistanceFragment();
        ...
        mNrdXRecyclerView.setLoadingListener(new XRecyclerView.LoadingListener() {
            ...
            public void onRefresh() {
                if (null != searchAddressList && searchAddressList.size() > 0) {
                    /*
                     * Scrapped or attached views may not be recycled. isScrap:false isAttached:true com.jcodecraeer.xrecyclerview.XRecyclerView
                     * 清空已有数据然后再获取网络接口数据操作，而从清空数据到获取到数据这段时间，List中的数据不存在，给了RecyclerView要回收Item，但View并未被回收的假象（此时并没有执行notifyDataSetChanged()），程序报错
                     * 将列表清空方法放到获取到接口数据以后执行
                     */
                    /*searchAddressList.clear();*/
        ...
        mNrdXRecyclerView.addOnItemTouchListener(new OnRecyclerItemClickListener(mNrdXRecyclerView) {
            ...
            public void onItemClick(RecyclerView.ViewHolder viewHolder) {
                ...
                if (viewHolder.itemView.getTag() != null) {
                    ...
                    Intent intent = new Intent(mActivity, UncertainEvidence.class);
            ...
            public void onSuccess(int statusCode, BaseBeans<List<TaskListBean.TaskList>> response) {
                ...
                if (null != data && data.size() > 0) {
                    if (pagecount == 1) {
                        ...
                        /*mAdapter.notifyItemRangeChanged(startPosition, searchAddressList.size() - 1);*/
                        mAdapter.notifyDataSetChanged();
                    }
                } else {
    ...
    public void onPause() {
        super.onPause();
        // 点进任务查看再返回当页只调用notifyDataSetChanged()
        // SharedPreferencesUtil.saveBoolean(getContext(), layoutmark, false);
    }
